...
export async function processSeedInboxes(): Promise<{
  ...
  const senders = await prisma.emailAccount.findMany({
    where: { status: "active" },
    select: { id: true, email: true, readEmulation: true, warmupOpenRate: true, warmupSpamProtection: true, warmupMarkImportant: true },
  });
  const senderEmails = senders.map(s => s.email);
  const senderIdByEmail = new Map(senders.map(s => [s.email, s.id]));
  const senderSettings = new Map(senders.map(s => [s.id, { openRate: s.warmupOpenRate ?? 100, spamProtection: s.warmupSpamProtection ?? 100, markImportant: s.warmupMarkImportant ?? 0, readEmulation: s.readEmulation ?? false }]));
