...
export default function AccountsConnector({
  variant,
  open,
  onClose,
  onSaved,
  ...
  const baseApi = isSeed ? "/api/admin/seeds" : "/api/email-accounts";
  const googleAuthPath = isSeed ? "/api/admin/seeds/google/auth" : "/api/email-accounts/google/auth";
  const microsoftAuthPath = isSeed ? "/api/admin/seeds/microsoft/auth" : "/api/email-accounts/microsoft/auth";
  const googleSource = isSeed ? "seed-google" : "google";
  const msSource = isSeed ? "seed-microsoft" : "microsoft";
  ...
  function resetForms() {
    ...
    setGoogleOAuthStatus("idle"); setGoogleOAuthMsg("");
  ...
  useEffect(() => {
    if (open) { setScreen("select"); resetForms(); }
  }, [open]);

  function connectAccount(data: any) {
    const payload = isSeed
      ? {
          email: data.email,
          provider: (data.provider === "Outlook" ? "outlook" : data.provider === "Gmail" ? "gmail" : data.provider || "other"),
          displayName: data.displayName,
          smtpHost: data.smtpHost, smtpPort: data.smtpPort,
          smtpUser: data.smtpUser, smtpPass: data.smtpPass,
          imapHost: data.imapHost, imapPort: data.imapPort,
          imapUser: data.imapUser, imapPass: data.imapPass,
        }
      : data;
    ...
    return fetch(baseApi, {
      ...
      if (!res.ok) { const err = await res.json(); throw new Error(err.error || "Failed to connect"); }
      return res.json();
    });
  }
  ...
  useEffect(() => {
    function handler(event: MessageEvent) {
      if (event.data?.source === googleSource) {
        if (event.data.success) {
          setGoogleOAuthStatus("success"); setGoogleOAuthMsg("Account connected!");
          savedRef.current();
          setTimeout(() => closeAndReset(), 1500);
        } else {
          setGoogleOAuthStatus("error"); setGoogleOAuthMsg(event.data.error || "Failed to connect Google account.");
          setTimeout(() => setGoogleOAuthStatus("idle"), 4000);
        }
      } else if (event.data?.source === msSource) {
        if (event.data.success) {
          setMsOAuthStatus("success"); setMsOAuthMsg("Account connected!");
          savedRef.current();
          setTimeout(() => closeAndReset(), 1500);
        } else {
  ...
  function openOAuth(path: string, setConnecting: (s: "idle" | "connecting" | "success" | "error") => void, setMsg: (m: string) => void) {
    const width = 600, height = 700;
    const left = (window.screen.width - width) / 2;
    const top = (window.screen.height - height) / 2;
    setConnecting("connecting"); setMsg("Opening login...");
  ...
  async function handleMicrosoftConnect() {
    ...
    await new Promise(r => setTimeout(r, 600));
    try {
      await connectAccount({ email: msEmail, provider: "Outlook", smtpHost: "smtp.office365.com", smtpPort: 587, smtpUser: msEmail, smtpPass: cleanPass, imapHost: "outlook.office365.com", imapPort: 993, imapUser: msEmail, imapPass: cleanPass, dailySendLimit: 50 });
    } catch (e: any) {
      setTestingError(e.message || "Failed to save account"); return;
    }
  ...
  async function handleGoogleAppPassword() {
    ...
    await new Promise(r => setTimeout(r, 600));
    try {
      await connectAccount({ email: gmailEmail, provider: "Gmail", smtpHost: "smtp.gmail.com", smtpPort: 587, smtpUser: gmailEmail, smtpPass: cleanPass, dailySendLimit: 50, displayName: gmailName || undefined });
    } catch (e: any) {
      setTestingError(e.message || "Failed to save account"); return;
    }
    setTestingStatus(prev => [...prev, "Account connected successfully!"]); setTestingDone(true);
  ...
  async function testImapConnection(navigateOnSuccess?: boolean) {
    ...
    try {
      ...
      if (data.success) {
        ...
        setImapStatus("error"); setImapStatusMsg(data.error || "IMAP connection failed");
        setTimeout(() => setImapStatus("idle"), 4000);
      }
    } catch {
      setImapStatus("error"); setImapStatusMsg("IMAP connection failed");
      setTimeout(() => setImapStatus("idle"), 4000);
    }
  }

  function testImapConnectionNavigate() { testImapConnection(true); }
  ...
  async function handleAnyConnect() {
    ...
    try {
      await connectAccount({ email: singleEmail, provider: "IMAP", smtpHost: smtpForm.host, smtpPort: parseInt(smtpForm.port) || 587, smtpUser: smtpUsername, smtpPass: smtpPassword, imapHost: imapForm.host, imapPort: parseInt(imapForm.port) || 993, imapUser: imapForm.username, imapPass: imapForm.password, dailySendLimit: 30, displayName: singleName || undefined });
    } catch (e: any) {
      setSmtpStatus("error"); setSmtpStatusMsg(e.message || "Failed to save account");
      setTimeout(() => setSmtpStatus("idle"), 4000); return;
    }
  ...
  async function handleBulkImport() {
    ...
    for (const acc of valid) {
      try {
        const res = await fetch(baseApi, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(isSeed ? { email: acc.email, provider: "other", smtpHost: acc.smtpHost || undefined, smtpPort: acc.smtpPort ? parseInt(acc.smtpPort) : undefined, smtpUser: acc.smtpUsername || acc.email, smtpPass: acc.smtpPassword || undefined, imapHost: acc.imapHost || undefined, imapPort: acc.imapPort ? parseInt(acc.imapPort) : undefined, imapUser: acc.imapUsername || acc.email, imapPass: acc.imapPassword || undefined, displayName: [acc.firstName, acc.lastName].filter(Boolean).join(" ") || undefined } : { email: acc.email, provider: "IMAP", smtpHost: acc.smtpHost || undefined, smtpPort: acc.smtpPort ? parseInt(acc.smtpPort) : undefined, smtpUser: acc.smtpUsername || acc.email, smtpPass: acc.smtpPassword || undefined, imapHost: acc.imapHost || undefined, imapPort: acc.imapPort ? parseInt(acc.imapPort) : undefined, imapUser: acc.imapUsername || acc.email, imapPass: acc.imapPassword || undefined, dailySendLimit: 30, displayName: [acc.firstName, acc.lastName].filter(Boolean).join(" ") || undefined }) });
    ...
    if (success > 0) savedRef.current();
    setBulkImporting(false); setScreen("any-bulk-results");
  }

  if (!open) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center" style={{ background: "rgba(15,13,20,0.4)", backdropFilter: "blur(4px)" }} onClick={closeAndReset}>
      <div className="bg-cream border border-border rounded-lg w-[90%] max-w-[440px] shadow-xl flex flex-col" onClick={e => e.stopPropagation()} style={{ maxHeight: "90vh" }}>
        <div className="flex items-center gap-3 px-8 pt-7 pb-4 shrink-0">
          ...
          <button onClick={closeAndReset} className="text-muted hover:text-blue-accent transition-colors shrink-0">
          ...
          <div className="px-8 pb-8 overflow-y-auto">
            ...
            {msOAuthStatus !== "idle" && (
              <div className={`flex items-center gap-2 px-4 py-3 rounded-lg text-sm mb-4 ${msOAuthStatus === "connecting" ? "bg-blue-50 text-blue-700 border border-blue-200" : msOAuthStatus === "success" ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-700 border border-red-200"}`}>
                {msOAuthStatus === "connecting" && <Spinner />}
        ...
        {screen === "any-imap" && (
          <div className="px-8 pb-8 overflow-y-auto">
            ...
            <div className="space-y-4">
              <Field label="IMAP Username *"><input value={imapForm.username} onChange={e => setImapForm(prev => ({ ...prev, username: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder={singleEmail || "user@website.com"} /></Field>
              <Field label="IMAP Password *"><input type="password" value={imapForm.password} onChange={e => setImapForm(prev => ({ ...prev, password: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder="••••••••" /></Field>
              <Field label="IMAP Host *"><input value={imapForm.host} onChange={e => setImapForm(prev => ({ ...prev, host: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder="imap.website.com" /></Field>
              <Field label="IMAP Port *"><input value={imapForm.port} onChange={e => setImapForm(prev => ({ ...prev, port: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder="993" /></Field>
            </div>
            <button onClick={() => { if (imapStatus !== "testing") testImapConnectionNavigate(); }} className="btn btn-primary w-full mt-6">{imapStatus === "testing" ? "Testing..." : imapStatus === "success" ? "Success! Continue..." : "Next >"}</button>
          </div>
        )}
        ...
        {screen === "any-smtp" && (
          <div className="px-8 pb-8 overflow-y-auto">
            ...
            <div className="space-y-4">
              <Field label="SMTP Username *"><input value={smtpForm.username} onChange={e => setSmtpForm(prev => ({ ...prev, username: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder={imapForm.username || "user@website.com"} /></Field>
              <Field label="SMTP Password *"><input type="password" value={smtpForm.password} onChange={e => setSmtpForm(prev => ({ ...prev, password: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder="••••••••" /></Field>
              <Field label="SMTP Host *"><input value={smtpForm.host} onChange={e => setSmtpForm(prev => ({ ...prev, host: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder="smtp.website.com" /></Field>
              <div className="grid grid-cols-2 gap-4">
                <Field label="SMTP Port *"><input value={smtpForm.port} onChange={e => setSmtpForm(prev => ({ ...prev, port: e.target.value }))} className="w-full bg-transparent border-b border-border pb-2.5 text-sm outline-none focus:border-blue-accent" placeholder="587" /></Field>
                <Field label="Encryption"><Select value={smtpForm.encryption} onChange={v => setSmtpForm(prev => ({ ...prev, encryption: v as Encryption }))} options={[{ value: "none", label: "None" }, { value: "ssl", label: "SSL/TLS" }, { value: "starttls", label: "STARTTLS" }]} placeholder="Select encryption" /></Field>
              </div>
            </div>
              ...
              {testingStatus.map((msg, i) => {
                ...
                return (
                  <div key={i} className="flex items-center gap-3 text-sm">
                    {isError ? <CircleXIcon size={16} style={{ color: "#DC2626" }} /> : isDone ? <CircleCheckIcon size={16} style={{ color: "#16A34A" }} /> : <Spinner />}
                    <span className={isDone ? "text-ink font-medium" : isError ? "text-red-600" : "text-muted"}>{msg}</span>
                  </div>
                );
              })}
            </div>
            {testingDone && !testingError && <button onClick={closeAndReset} className="btn btn-primary w-full">Done</button>}
        ...
        {screen === "any-bulk-results" && bulkResults && (
          <div className="px-8 pb-8 overflow-y-auto">
            ...
            <button onClick={closeAndReset} className="btn btn-primary w-full">Done</button>
          </div>
        )}
      </div>
    </div>
...
function Spinner() {
  return <svg className="animate-spin" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" strokeDasharray="31.4 31.4" strokeLinecap="round" /></svg>;
}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div className="field-group"><label className="block text-xs text-muted mb-2">{label}</label>{children}</div>;
}
