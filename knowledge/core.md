...
- **Inbox warmup** increases your sender reputation gradually and safely, so
  your emails stop landing in spam and start reaching real inboxes.
- **Lead verification** checks every email address before sending, so you don't
  burn sends (or your reputation) on bad addresses.
- **Scheduling** lets you control exactly when sending happens, so you only
  "show up" during business hours or your chosen window.
- **Deliverability checks** scan every outgoing email for spam-trigger words
  (free, guarantee, act now, ALL-CAPS, etc.) before it leaves.

## Key concepts used throughout

- **Leads** = the contacts you're emailing.
- **Campaigns** = the structured sequence of emails you send to leads.
- **Email accounts / inboxes** = the real mailboxes you've connected.
- **Credits** = the metered unit that powers sending, verification, AI, and
  lead import.
- **Warmup** = automated reputation building on a connected inbox.
